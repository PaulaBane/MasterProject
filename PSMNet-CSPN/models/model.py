...
class Model:
    def __init__(self, sess, height=256, weight=512, batch_size=4, max_disp=129, lr=0.001):
    ...
    def build_model(self):
        ...
        output1 = self.CNN3D(cost_vol)

        blur_depth = up_proj_block(output1, 61, 1, 228, 304)
        sparse_depth = tf.identity(output1.narrow(1,3,1))
        outputs = Affinity_Propagate(output1, blur_depth, sparse_depth, 24, 3, '8sum')
    ...
    def cost_vol(self, left, right, max_disp=192):
        with tf.variable_scope('cost_vol'):
            ...
            right_tensor = K.spatial_2d_padding(right, padding=((0, 0), (max_disp // 4, 0)))
            disparity_costs = []
