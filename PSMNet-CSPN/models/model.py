...
class Model:
    ...
    def cspn(self, bottom):
        #print(bottom.shape)
        with tf.compat.v1.variable_scope('Affinity'):
            bottom=Affinity_Propagate()(tf.squeeze(bottom, [0]))
        return bottom
    ...
    def CNN(self, bottom, reuse=False):
        with tf.compat.v1.variable_scope('CNN'):
            with tf.compat.v1.variable_scope('conv0'):
                bottom = conv_block(tf.keras.layers.Conv2D, bottom, 32, 3, strides=2, name='conv0_1', reuse=reuse, reg=self.reg)
                for i in range(1, 3):
                    bottom = conv_block(tf.keras.layers.Conv2D, bottom, 32, 3, name='conv0_%d' % (i+1), reuse=reuse, reg=self.reg)
            with tf.compat.v1.variable_scope('conv1'):
                for i in range(3):
                    bottom = res_block(tf.keras.layers.Conv2D, bottom, 32, 3, name='conv1_%d' % (i+1), reuse=reuse, reg=self.reg)
            with tf.compat.v1.variable_scope('conv2'):
                bottom = res_block(tf.keras.layers.Conv2D, bottom, 64, 3, strides=2, name='conv2_1', reuse=reuse, reg=self.reg,
                                   projection=True)
                for i in range(1, 8):
                    bottom = res_block(tf.keras.layers.Conv2D, bottom, 64, 3, name='conv2_%d' % (i+1), reuse=reuse, reg=self.reg)
            with tf.compat.v1.variable_scope('conv3'):
                bottom = res_block(tf.keras.layers.Conv2D, bottom, 128, 3, dilation_rate=2, name='conv3_1', reuse=reuse,
                                   reg=self.reg, projection=True)
                for i in range(1, 3):
                    bottom = res_block(tf.keras.layers.Conv2D, bottom, 128, 3, dilation_rate=2, name='conv3_%d' % (i+1), reuse=reuse,
                                       reg=self.reg)
            with tf.compat.v1.variable_scope('conv4'):
                for i in range(3):
                    bottom = res_block(tf.keras.layers.Conv2D, bottom, 128, 3, dilation_rate=4, name='conv4_%d' % (i+1), reuse=reuse,
                                       reg=self.reg)
        return bottom
    ...
    def SPP(self, bottom, reuse=False):
        with tf.compat.v1.variable_scope('SPP'):
            ...
            for i, p in enumerate([64, 32, 16, 8]):
                branches.append(SPP_branch(tf.keras.layers.Conv2D, bottom, p, 32, 3, name='branch_%d' % (i+1), reuse=reuse,
                                           reg=self.reg))
            ...
            with tf.compat.v1.variable_scope('fusion'):
                bottom = conv_block(tf.keras.layers.Conv2D, concat, 128, 3, name='conv1', reuse=reuse, reg=self.reg)
                fusion = conv_block(tf.keras.layers.Conv2D, bottom, 32, 1, name='conv2', reuse=reuse, reg=self.reg)
        return fusion
    ...
    def CNN3D(self, bottom):
        with tf.compat.v1.variable_scope('CNN3D'):
            for i in range(6):
                bottom = conv_block(tf.keras.layers.Conv3D, bottom, 32, 3, name='3Dconv0_%d' % (i+1), reg=self.reg)

            out = conv_block(tf.keras.layers.Conv3D, bottom, 1, 3, name='3Dconv5', reg=self.reg)
        return out
    ...
    def resnet_3d(self, bottom):
        with tf.compat.v1.variable_scope('RES3D'):
            ...
            _3Dconv1 = res_block(tf.keras.layers.Conv3D, bottom, 32, 3, name='3Dconv1', reg=self.reg)
            _3Dconv1 = res_block(tf.keras.layers.Conv3D, _3Dconv1, 32, 3, name='3Dconv2', reg=self.reg)
            _3Dconv1 = res_block(tf.keras.layers.Conv3D, _3Dconv1, 32, 3, name='3Dconv3', reg=self.reg)
            _3Dconv1 = res_block(tf.keras.layers.Conv3D, _3Dconv1, 32, 3, name='3Dconv4', reg=self.reg)
            out = conv_block(tf.keras.layers.Conv3D, _3Dconv1, 1, 3, name='3Dconv5', reg=self.reg)
        return out
    ...
    def densenet_3d(self, bottom):
        with tf.compat.v1.variable_scope('DENSE3D'):
            ...
            out = [bottom]
            for j in range(5):
                bottom = conv_block(tf.keras.layers.Conv3D, bottom, 32, 3, name='3Dconv1_%d' % (j+1), reg=self.reg)
                out.append(bottom)
    ...
    def output(self, output):
        ...
        upsample = tf.image.resize(upsample, tf.constant([self.max_disp, self.height], dtype=tf.int32))
